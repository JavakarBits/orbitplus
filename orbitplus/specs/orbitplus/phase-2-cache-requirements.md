...
- `trip:{operatorCode}:{tripCode}`: canonical common Trip content, owned and written only by SEARCH ingestion.
- `stage:{operatorCode}:{tripCode}:{tripStageCode}`: canonical Stage-specific content, owned and written only by SEARCH ingestion.
...
The Trip value may contain supplied common information such as `tripCode`, `travelDate`, `tripDate`, `displayName`, `travelTime`, `closeTime`, `bus`, `schedule`, `tripStatus`, `operator`, `amenities`, `activities`, `cancellationTerm`, `additionalAttributes`, and `viaStations`. The Stage value may contain supplied Stage information such as `tripStageCode`, `fromStation`, `toStation`, `stationPoint`, and `stageFare[]`. The BUSMAP value retains the complete supplied BUSMAP entry so its API-specific values, formatting, array order, optional fields, and unknown fields can be reproduced independently of SEARCH.

These examples do not define a rigid schema or unconditional ownership. A field is common or Stage-specific only when the approved lossless ownership rule identifies it from the received structure. The implementation must not assume that a field is common merely because it appears in an example, silently discard it, synthesize it, or duplicate an entire SEARCH response into every Stage.
...
5. SEARCH alone writes the losslessly owned common content to the Trip key and Stage content to its distinct Stage key.
...
A SEARCH response may contain many `data[]` entries for the same `tripCode` and different `tripStageCode` values. The Master SHALL retain common content once in `trip:{operatorCode}:{tripCode}` and retain each supplied Stage independently in `stage:{operatorCode}:{tripCode}:{tripStageCode}`. A Stage write must never overwrite another Stage. SEARCH must preserve all supplied `stageFare[]` values and must not fabricate `seatLayoutList[]`.
...
Phase 2 storage shall preserve arbitrary nested and unknown Bits fields. It shall not rename, remove, default, coerce, or semantically validate fields. It shall not create fake stages, empty arrays, empty layouts, or seat records. Unknown-field ownership during common/Stage/BUSMAP separation must be lossless; if structural preservation cannot be guaranteed by an approved rule, the data must not be silently split or discarded.

Trip content must not be unnecessarily duplicated in each Stage. Stage and BUSMAP data for different `tripStageCode` values must remain isolated. Cassandra must never become a fallback content store.
...
- The Trip key stores a common SEARCH projection. The Stage key stores the supplied SEARCH Trip entry for its `tripStageCode`, excluding `bus.seatLayoutList`. The matching BUSMAP key stores the complete supplied BUSMAP response entry and is served independently to preserve BUSMAP contract parity.
