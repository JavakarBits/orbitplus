...
The metadata access patterns support `operatorCode + travelDate + fromStationCode + toStationCode → one or more tripCode/tripStageCode` and `operatorCode + scheduleCode + travelDate → one or more tripCode/tripStageCode`. BUSMAP uses the route lookup and selects the requested `tripCode` in the application. The precise Cassandra partitioning and clustering are design decisions; complete JSON is prohibited in all designs.
...
- Cassandra metadata uses two active indexes: a route index for `operatorCode + travelDate + fromStationCode + toStationCode`, and a schedule index for `operatorCode + scheduleCode + travelDate`. BUSMAP filters route candidates by the known `tripCode` in the application. The legacy `trip_details_metadata_by_stage` table is not used or created by this version and may be removed only through an explicitly approved migration.
